...
{
	Mat image = imread("car.jpg", IMREAD_GRAYSCALE);
	Mat imageNormalised;
	Mat imageEqualised;
	int histSize = 256;
	...
	imshow("Зображення", image);
	imshow("Гістограма зображення", histImage);
	imshow("Зображення з еквалізованою гістограмою", imageEqualised);
	imshow("Нормалізоване зображення", imageNormalised);
	waitKey(0);
	return 0;
}